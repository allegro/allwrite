...
internal fun KotlinListLiteral(
    prefix: Space = Space.EMPTY,
    markers: Markers = Markers.EMPTY,
    elements: List<Expression>,
    type: JavaType?,
): K.ListLiteral = K.ListLiteral(Tree.randomId(), prefix, markers, JContainer.build(elements.map { JRightPadded.build(it) }), type)

internal fun KotlinStringListLiteral(
    prefix: Space = Space.EMPTY,
    markers: Markers = Markers.EMPTY,
    values: List<String>
...
internal fun GroovyListLiteral(
    prefix: Space = Space.EMPTY,
    markers: Markers = Markers.EMPTY,
    elements: List<Expression>,
    type: JavaType?,
): G.ListLiteral = G.ListLiteral(Tree.randomId(), prefix, markers, JContainer.build(elements.map { JRightPadded.build(it) }), type)

internal fun GroovyStringListLiteral(
    prefix: Space = Space.EMPTY,
    markers: Markers = Markers.EMPTY,
    values: List<String>
