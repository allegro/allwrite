...
public class RemoveUnusedPrivateFields extends Recipe {
    ...
    String[] onlyRemoveFieldsOfType;

    public RemoveUnusedPrivateFields() {
    }

    public RemoveUnusedPrivateFields(String[] onlyRemoveFieldsOfType) {
        this.onlyRemoveFieldsOfType = onlyRemoveFieldsOfType;
    }

    public String[] getOnlyRemoveFieldsOfType() {
        return onlyRemoveFieldsOfType;
    }

    @Option
    public void setOnlyRemoveFieldsOfType(String[] onlyRemoveFieldsOfType) {
        this.onlyRemoveFieldsOfType = onlyRemoveFieldsOfType;
    }
    ...
    public TreeVisitor<?, ExecutionContext> getVisitor() {
        return new JavaIsoVisitor<ExecutionContext>() {
            class CheckField {
                ...
                public CheckField(J.VariableDeclarations declarations, @Nullable Statement nextStatement) {
            ...
            public J.ClassDeclaration visitClassDeclaration(J.ClassDeclaration classDecl, ExecutionContext ctx) {
                ...
                Set<String> fieldTypesToRemove = Optional.ofNullable(onlyRemoveFieldsOfType).map(Set::of).orElse(null);
                ...
                for (int i = 0; i < statements.size(); i++) {
                    ...
                    if (statement instanceof J.VariableDeclarations) {
                        ...
                        boolean fieldTypeShouldBeRemoved = true;

                        if (fieldTypesToRemove != null) {
                            fieldTypeShouldBeRemoved = vd.getVariables().stream()
                                .filter(it -> it.getType() instanceof JavaType.FullyQualified)
                                .anyMatch(it -> fieldTypesToRemove.contains(((JavaType.FullyQualified) it.getType()).getFullyQualifiedName()));
                        }
