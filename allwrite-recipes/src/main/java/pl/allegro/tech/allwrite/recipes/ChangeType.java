...
public class ChangeType extends Recipe {
    ...
    public ChangeType(String oldFullyQualifiedTypeName, String newFullyQualifiedTypeName, Boolean ignoreDefinition) {
    ...
    public void setOldFullyQualifiedTypeName(String oldFullyQualifiedTypeName) {
        this.oldFullyQualifiedTypeName = oldFullyQualifiedTypeName;
    }
    ...
    public void setNewFullyQualifiedTypeName(String newFullyQualifiedTypeName) {
        this.newFullyQualifiedTypeName = newFullyQualifiedTypeName;
    }
    ...
    @Option(displayName = "Ignore type definition",
        ...
        required = false)
    @Nullable
    public void setIgnoreDefinition(@Nullable Boolean ignoreDefinition) {
        this.ignoreDefinition = ignoreDefinition;
    }
    ...
    public TreeVisitor<?, ExecutionContext> getVisitor() {
        TreeVisitor<?, ExecutionContext> condition = new TreeVisitor<Tree, ExecutionContext>() {
            @Override
            public Tree preVisit(@Nullable Tree tree, ExecutionContext ctx) {
                stopAfterPreVisit();
        ...
        return Preconditions.check(condition, new JavaChangeTypeVisitor(oldFullyQualifiedTypeName, newFullyQualifiedTypeName, ignoreDefinition));
    }
    ...
    private static class JavaChangeTypeVisitor extends JavaVisitor<ExecutionContext> {
        ...
        public J visitFieldAccess(J.FieldAccess fieldAccess, ExecutionContext ctx) {
            if (fieldAccess.isFullyQualifiedClassReference(originalType.getFullyQualifiedName())) {
                if (targetType instanceof JavaType.FullyQualified fullyQualified) {
                    // Use simple type name for variable declarations and other places of normal use in method body,
                    // fully qualified name for imports, etc.
                    J.FieldAccess typeTree = TypeTree.build(fullyQualified.getFullyQualifiedName());
                    if (getCursor().getParent() != null
                        && Set.of(J.VariableDeclarations.class, J.NewClass.class, J.EnumValue.class)
                            .contains(getCursor().getParent().getValue().getClass())
                    ) {
                        getCursor().getRoot().computeMessageIfAbsent("importsToAdd", key -> new HashSet<String>()).add(fullyQualified.getFullyQualifiedName());
                        return new J.Identifier(randomId(), fieldAccess.getPrefix(), fieldAccess.getMarkers(), emptyList(), fullyQualified.getClassName(), typeTree.getType(), null);
                    } else {
                        return updateOuterClassTypes(typeTree).withPrefix(fieldAccess.getPrefix());
                    }
        ...
        public J visitIdentifier(J.Identifier ident, ExecutionContext ctx) {
            // Do not modify the identifier if it's on an inner class definition.
            ...
            if (TypeUtils.isOfClassType(ident.getType(), originalType.getFullyQualifiedName())) {
                ...
                if (ident.getSimpleName().equals(className)) {
                    if (targetType instanceof JavaType.FullyQualified fullyQualified) {
                        if (fullyQualified.getOwningClass() != null) {
                            return updateOuterClassTypes(TypeTree.build(fullyQualified.getClassName())
                                .withType(null)
                                .withPrefix(ident.getPrefix()));
                        } else {
                            ident = ident.withSimpleName(fullyQualified.getClassName());
                        }
                ...
                if (ident.getSimpleName().equals(decapitalize(className))) {
                    if (targetType instanceof JavaType.FullyQualified fullyQualified) {
                        String targetVariableName = decapitalize(fullyQualified.getClassName());
                        ident = ident
                            .withSimpleName(targetVariableName)
                            .withType(targetType);

                        if (ident.getFieldType() != null) {
                            ident = ident.withFieldType(
                                ident.getFieldType()
                                    .withName(targetVariableName)
                                    .withType(targetType)
                            );
                        }
                    } else if (targetType instanceof JavaType.Primitive primitive) {
                        String targetVariableName = decapitalize(primitive.getKeyword());
                        ident = ident
                            .withSimpleName(targetVariableName)
                            .withType(targetType);
