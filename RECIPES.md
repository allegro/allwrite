...
### `pl.allegro.tech.allwrite.recipes.gradle.dependencyupdate.RegexpDependencyUpdate`
...
- String notation: `classpath("GROUP:ID:1.4.46")`
- Positional arguments: `classpath("GROUP", "ID", "1.4.40")`
- Groovy map notation: `classpath group: 'GROUP', name: 'ID', version: '1.4.32'`
- Kotlin named arguments: `classpath(group = "GROUP", name = "ID", version = "1.4.42")`
...
| `multiline` | `Boolean` | No | Enable multiline regex mode. Defaults to `true`. |
| `updateOnlyWhenMainRecipeMadeChanges` | `Boolean` | No | Only apply the update if a preceding recipe made changes. Defaults to `false`. |

Before (with `groupId = "pl.allegro.tech.phoenix"`, `artifactId = "phoenix-provisioning-plugin"`, `targetVersion = "2.0.0"`):
```groovy
classpath("pl.allegro.tech.phoenix:phoenix-provisioning-plugin:1.4.46")
```

After:
```groovy
classpath("pl.allegro.tech.phoenix:phoenix-provisioning-plugin:2.0.0")
```
...
### `pl.allegro.tech.allwrite.recipes.java.ChangeRecordFieldName`
...
```java
public record SomeRecord(List<String> oldName) { }
```

than has been changed to:
```java
public record SomeRecord(List<String> newName) { }
```
...
```java
class Foo {
    List<String> foo(SomeRecord someRecord) {
...
```kotlin
fun foo(someRecord: SomeRecord): List<String> {
    return someRecord.oldName
}
```
